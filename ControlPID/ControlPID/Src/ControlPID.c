 ...
 * C/C++ source code generated on  : Thu Dec  2 16:23:29 2021
 *
...
void ControlPID_step(void)
{
  ...
  if (ControlPID_DW.stateControl == 1.0) {
    ...
    rtb_Sum_p = ControlPID_DW.Ref - ControlPID_DW.rads;
    rtb_TSamp = 0.0008954 * rtb_Sum_p * 0.089540000000000008;
    rtb_Sum5 = ((rtb_TSamp - ControlPID_DW.UD_DSTATE) + 0.0121 * rtb_Sum_p) +
    ...
    if (rtb_Sum5 > 8.37) {
      ...
      rtb_Saturation1 = rtb_Sum5;
    }

    ControlPID_B.Divide = 100.0 * rtb_Saturation1 / 8.37;
    ...
    ControlPID_DW.DiscreteTimeIntegrator_DSTATE += ((rtb_Saturation1 - rtb_Sum5)
      * 32.231404958677686 + 0.39 * rtb_Sum_p) * 0.01;

    {
      if (ControlPID_B.Divide != TIM15_Conf.CH2_duty) {
        TIM15_Conf.CH2_duty = ControlPID_B.Divide;
        if (ControlPID_B.Divide >= 0) {
          /* Channel2 duty cycle is an input port. */
          __HAL_TIM_SET_COMPARE(&htim15,TIM_CHANNEL_2,(uint32_t)
                                (ControlPID_B.Divide * (&htim15)->Instance->ARR /
            100));
        }
      }
    }
